  ...
  <div>
    ...
    <el-card class="box-card">
      ...
      >
          ...
          <template slot="header" @click="Participants">
            <el-dropdown>
              ...
              <el-dropdown-menu slot="dropdown">
                <el-dropdown-item
                  @click.native="selectFinishStart"
                  style="font-size: 10px"
                  >未开始</el-dropdown-item
                >
                <el-dropdown-item
                  @click.native="selectFinishProcess"
                  style="font-size: 10px"
                  >进行中</el-dropdown-item
                >
                <el-dropdown-item
                  @click.native="selectFinishEnd"
                  style="font-size: 10px"
        ...
        <el-table-column label="任务说明" prop="task_Description">
          <template slot-scope="scope">
            <div @click="taskDescip(scope.row.task_Description, scope.$index)">
        ...
        <el-table-column label="操作" width="228px">
          <template slot="header">
            <span class="task_Finish_Dropdown1"> 操作 </span>
          </template>
          ...
          <template slot-scope="scope">
            ...
            >
              <el-button
                @click="taskFinished(scope.$index)"
                type="success"
                icon="el-icon-setting"
                size="mini"
              ></el-button>
    ...
    >
      ...
      >
        ...
        <el-form-item label="起止日期" prop="task_Period">
          <el-date-picker
            @change="recoveryUserlist()"
            clearable
            v-model="addForm.task_Period"
            type="daterange"
            align="right"
            unlink-panels
            range-separator="至"
...
export default {
  ...
  created() {
    ...
    this.userList=user
    //alert(this.userList)
    for(var i=0;i<this.userList.length
    ;i++){
      if(i<this.options1.length){
       this.options1[i].value=this.userList[i].name+'\n'
       this.options1[i].label=this.userList[i].name
      }else{
      const obj = {
        value:this.userList[i].name+'\n',
        label:this.userList[i].name
      }
      this.options1.push(obj)
    }
    }
    this.limitTime0 = this.course.courseYear
    this.limitTime1 = this.course.courseTerm

    this.limitTime0 = new Date(this.limitTime0).getTime() / 1000
    this.limitTime1 = new Date(this.limitTime1).getTime() / 1000
    // alert(this.limitTime0)
    // alert(this.limitTime1)
    //alert(id);
    //console.log(id);
  },
  ...
  methods: {
    ...
    // 点击按钮，添加新用户
    addUser() {
      this.addForm.task_StartTime = this.addForm.task_Period[0]
      this.addForm.task_EndTime = this.addForm.task_Period[1]
      let taskStartTime = new Date(
                this.addForm.task_StartTime 
                //  "1970/01/01 " + this.taskList[i].task_StartTime + ":00"
              ).getTime()/1000;
      let taskEndTime = new Date(
                this.addForm.task_EndTime 
                //  "1970/01/01 " + this.taskList[i].task_StartTime + ":00"
              ).getTime()/1000;
      ...
      this.$refs.addFormRef.validate((valid) => {
        ...
        if(taskStartTime<this.limitTime0||taskEndTime>this.limitTime1){
          this.$message.error('任务时期需在项目时期中')
          return
        }
    ...
    showEditDialog(userindex) {
      ...
      this.editForm.task_Period = [
        this.taskList[userindex].task_StartTime,
        this.taskList[userindex].task_EndTime,
      ]
      //this.editForm.task_Period[1]=this.taskList[userindex].task_EndTime;
    ...
    editUserInfo(user_index) {
      ...
      this.editForm.task_StartTime = this.editForm.task_Period[0]
      this.editForm.task_EndTime = this.editForm.task_Period[1]

      let taskStartTime = new Date(
                this.editForm.task_StartTime 
                //  "1970/01/01 " + this.taskList[i].task_StartTime + ":00"
              ).getTime()/1000;
      let taskEndTime = new Date(
                this.editForm.task_EndTime 
                //  "1970/01/01 " + this.taskList[i].task_StartTime + ":00"
              ).getTime()/1000;
      ...
      this.$refs.editFormRef.validate((valid) => {
        ...
        this.taskList[user_index].task_Description =
    ...
    selectFinishProcess() {
      ...
      for (var i = 0; i < this.taskList.length; i++) {
        if (
          this.taskList[i].task_Finish > 0 &&
          this.taskList[i].task_Finish < 100
        ) {
          this.searchList.push(this.taskList[i])
        }
      }
